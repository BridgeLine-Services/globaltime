...
import { useStaticClock } from '../hooks/useLiveClock';
...
export const CountryCard: React.FC<CountryCardProps> = ({ country, compact = false }) => {
  // Cards use the cheap 1s clock — no milliseconds shown here.
  const time = useStaticClock(country.timezone);
  const isDay = isDaytime(country.timezone, time); // reuse snapshot
      ...
      <div className={`relative ${compact ? 'p-3' : 'p-4'}`}>
        <div className="flex items-start justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className={`text-white font-semibold ${compact ? 'text-sm' : 'text-base'} leading-tight`}>
                {country.name}
              </div>
        ...
        {/* Pass precise=false explicitly and showMs=false for card context */}
        <LiveClock
