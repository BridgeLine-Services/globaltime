...
export const CountryCard: React.FC<CountryCardProps> = ({ country, compact = false }) => {
  // useStaticClock now owns isDay — no separate isDaytime() call needed.
  const { isDay } = useStaticClock(country.timezone);
  ...
  return (
    ...
    >
      ...
      <div className={`relative ${compact ? 'p-3' : 'p-4'}`}>
        ...
        {/* Cards use static 1s clock, no milliseconds */}
        <LiveClock timezone={country.timezone} size="sm" showMs={false} precise={false} />
