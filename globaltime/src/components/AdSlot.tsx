...
export const AdSlotComponent: React.FC<AdSlotProps> = ({ position, className = '', index = 0 }) => {
  ...
  const { recordAdClick } = useAnalyticsStore();
  ...
  return (
    ...
    >
      <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
        style={{ background: `linear-gradient(135deg, ${ad.textColor}15, transparent)` }} />
      ...
      <div className="absolute bottom-0 left-0 right-0 h-px opacity-30"
        style={{ background: `linear-gradient(90deg, transparent, ${ad.textColor}, transparent)` }} />
    </div>
  );
};
